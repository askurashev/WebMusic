...
check(!$('auto-save').checked && state.revision === revisionBeforeAutoSave && !$('save').disabled, 'autosave is off by default and edits wait for disk button');
$('auto-save').checked = true; $('auto-save').onchange();
await new Promise(resolve => setTimeout(resolve, 550));
...
click($('ranking').children[1].querySelector('.actions button'));
$('auto-save').checked = false; $('auto-save').onchange();
const revisionBeforeDisable = state.revision;
await new Promise(resolve => setTimeout(resolve, 550));
check(state.revision === revisionBeforeDisable && !$('save').disabled, 'turning Auto off cancels pending save and preserves edits');
await click($('save'));
check(state.revision === revisionBeforeDisable + 1 && $('save').disabled, 'disk button saves with Auto off');
...
filter('chart-filter', '');
const chartStar = path => [...$('library').children].find(row => row.querySelector('.info')?.dataset.path === path).querySelector('.chart-button');
check(chartStar(b).textContent === '★' && chartStar(b).getAttribute('aria-pressed') === 'true' && !chartStar(b).disabled, 'added track keeps a clickable active star');
click(chartStar(b));
check(!paths().includes(b) && chartStar(b).textContent === '★' && chartStar(b).getAttribute('aria-pressed') === 'false', 'star removes track and returns to inactive state');
click(chartStar(b));
check(paths().at(-1) === b && chartStar(b).getAttribute('aria-pressed') === 'true', 'inactive star adds track back to chart');
filter('chart-filter', 'in');
click(chartStar(b));
check(!libraryPaths().includes(b), 'star removal immediately updates chart membership filter');
filter('chart-filter', 'out');
