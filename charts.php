...
function cleanOwned(string $dir, string $parent): void {
    ...
    if (!$resolved || !inside($resolved, $parent)) throw new RuntimeException('Недопустимый путь папки экспорта.');
    // Keep the manifest until all audio files are removed, so a failed cleanup can be retried.
    $files = ownedFiles($dir);
    foreach (array_diff($files, ['.mfp-export.json']) as $name) {
        if (is_file($dir . '/' . $name) && !unlink($dir . '/' . $name))
            throw new RuntimeException('Не удалось очистить current. Закройте использующие её файлы программы и повторите экспорт.');
    }
    if (!unlink($dir . '/.mfp-export.json') || !rmdir($dir))
        throw new RuntimeException('Не удалось удалить папку current. Закройте использующие её программы.');
}
function exportChart(array $state, string $week, array $cfg): array {
    ...
    $published = false;
    try {
        // Release the old export's disk space before copying any new audio.
        if (is_dir($current)) cleanOwned($current, $parent);
        ...
        $published = true;
        return ['week' => $week, 'revision' => $state['revision'], 'path' => str_replace('\\', '/', $current), 'count' => count($files), 'warning' => ''];
    } finally {
...
try {
    ...
    if ($action === 'playlists' || $action === 'playlist-add' || $action === 'playlist-remove') {
        ...
        try {
            if ($action === 'playlist-add' || $action === 'playlist-remove') {
                ...
                if (($action === 'playlist-remove' || empty($input['create'])) && !file_exists($playlistFile)) fail('Плейлист удалён. Обновите список.', 404);
                $value = is_file($playlistFile) ? mfpReadPlaylist($playlistFile) : [];
                $entries = $value['playlist'] ?? $value;
                if ($action === 'playlist-remove') {
                    $remaining = array_values(array_filter($entries, fn($entry) => $entry['path'] !== $path));
                    if (isset($value['playlist'])) {
                        if (isset($value['index'])) {
                            $index = (int)$value['index'];
                            $before = count(array_filter(array_slice($entries, 0, max(0, $index)), fn($entry) => $entry['path'] === $path));
                            $value['index'] = max(-1, min($index - $before, count($remaining) - 1));
                        }
                        $value['playlist'] = $remaining;
                    } else $value = $remaining;
                    mfpWritePlaylist($playlistFile, $value);
                } elseif (!in_array($path, array_column($entries, 'path'), true)) {
                    $entry = ['path' => $path];
